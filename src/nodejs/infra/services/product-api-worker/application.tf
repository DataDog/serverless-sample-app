...
module "product_api_pricing_worker" {
  ...
  zip_file       = "../out/handlePricingChangedFunction/handlePricingChangedFunction.zip"
...
module "product_api_stock_updated_worker" {
  service_name   = "NodeProductApi"
  source         = "../../modules/lambda-function"
  zip_file       = "../out/handleStockLevelUpdatedFunction/handleStockLevelUpdatedFunction.zip"
  function_name  = "ProductStockLevelUpdatedHandlerFunction"
  lambda_handler = "index.handler"
  environment_variables = {
    TABLE_NAME : data.aws_ssm_parameter.product_api_table_name.value
...
resource "aws_lambda_permission" "product_stock_updated_sns_permissions" {
  statement_id  = "AllowExecutionFromSNS"
  action        = "lambda:InvokeFunction"
...
resource "aws_sns_topic_subscription" "product_stock_updated_topic_sub" {
