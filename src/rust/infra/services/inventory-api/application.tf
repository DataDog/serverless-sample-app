...
resource "aws_iam_role" "ecs_task_execution_role" {
  name = "RustInventoryApiTaskExecutionRole"
...
module "inventory_api_web_service" {
  ...
  service_name = "RustInventoryApi"
  image        = "public.ecr.aws/k4y9x2e7/dd-serverless-sample-app-rust:latest"
  env          = var.env
  app_version  = var.app_version
  environment_variables = [
    {
      name  = "TABLE_NAME"
      value = aws_dynamodb_table.rust_inventory_api.name
    },
    {
      name  = "EVENT_BUS_NAME"
...
resource "aws_ssm_parameter" "table_name_param" {
  name  = "/rust/inventory/${var.env}/table-name"
  type  = "String"
  value = aws_dynamodb_table.rust_inventory_api.name
}
