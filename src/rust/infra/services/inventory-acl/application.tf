...
resource "aws_sqs_queue" "public_event_acl_dlq" {
  name = "tf-rust-inventory-acl-dlq-${var.env}"
}

resource "aws_sqs_queue" "public_event_acl_queue" {
  name                      = "tf-rust-inventory-acl-${var.env}"
...
resource "aws_sns_topic" "rust_inventory_new_product_added" {
  name = "tf-rust-rust-inventory-new-product-added-${var.env}"
}
...
module "inventory_acl_function" {
  ...
  environment_variables = {
    PRODUCT_ADDED_TOPIC_ARN : aws_sns_topic.rust_inventory_new_product_added.arn
