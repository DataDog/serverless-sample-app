...
impl EventBridgeEventPublisher {
    pub  fn new (client: Client, event_bus_name: String, env: String) ->  Self {
        Self {client, event_bus_name, source: format!("{}.products", env)}
    }
    ...
    async fn send_to_event_bridge<T>(&self, detail_type: String, detail: T) -> Result<(), ()> where T : Serialize{
        ...
        let request = aws_sdk_eventbridge::types::builders::PutEventsRequestEntryBuilder::default()
            ...
            .build();
        self.client.put_events().entries(request).send().await.map_err(|err| {
            tracing::error!("{}", err);
            ()
        })?;
...
impl PublicEventPublisher for EventBridgeEventPublisher {
    async fn publish_created(&self, product_created_event_v1: ProductCreatedEventV1) -> Result<(), ()> {
        let detail_type = "product.productCreated.v1".to_string();
        
        self.send_to_event_bridge(detail_type, product_created_event_v1).await
    }

    async fn publish_updated(&self, product_updated_event_v1: ProductUpdatedEventV1) -> Result<(), ()> {
        let detail_type = "product.productUpdated.v1".to_string();

        self.send_to_event_bridge(detail_type, product_updated_event_v1).await
    }

    async fn publish_deleted(&self, product_deleted_event_v1: ProductDeletedEventV1) -> Result<(), ()> {
        let detail_type = "product.productDeleted.v1".to_string();

        self.send_to_event_bridge(detail_type, product_deleted_event_v1).await
    }
}