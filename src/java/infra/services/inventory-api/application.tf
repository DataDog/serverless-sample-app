...
resource "aws_iam_role" "ecs_task_execution_role" {
  name = "JavaInventoryApiTaskExecutionRole"
...
module "inventory_api_web_service" {
  ...
  environment_variables = [
    {
      name  = "TABLE_NAME"
      value = aws_dynamodb_table.java_inventory_api.name
    },
    {
      name  = "EVENT_BUS_NAME"
...
resource "aws_ssm_parameter" "table_name_param" {
  name  = "/java/inventory/${var.env}/table-name"
  type  = "String"
  value = aws_dynamodb_table.java_inventory_api.name
}
