...
resource "aws_iam_policy" "sfn_start_execution" {
  name   = "java-tf-inventory-ordering-service-start-execution-${var.env}"
  path   = "/"
  policy = data.aws_iam_policy_document.stepfunctions_start_execution.json
}

resource "aws_iam_role" "invetory_ordering_sfn_role" {
  name = "java-tf-inventory-ordering-service-sfn-role-${var.env}"
...
resource "aws_iam_policy" "function_logging_policy" {
  name = "java-tf-inventory-ordering-service-logging-policy-${var.env}"
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
...
resource "aws_iam_policy" "ddb_write_policy" {
  name = "tf-java-inventory-ordering-service-db-write-${var.env}"
  policy = jsonencode({
    "Version" : "2012-10-17",
    "Statement" : [
      {
