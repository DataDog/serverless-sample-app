...
public class ApiWorkerService extends Construct {
    public ApiWorkerService(@NotNull Construct scope, @NotNull String id, @NotNull ApiWorkerServiceProps props) {
        ...
        functionEnvVars.put("TABLE_NAME", props.getProductApiTable().getTableName());
        ...
        IFunction handlePriceCalculatedFunction = new InstrumentedFunction(this, "HandlePriceCalculatedFunction",
                new InstrumentedFunctionProps(props.getSharedProps(), "com.product.api", compiledJarFilePath, "handlePricingChanged", functionEnvVars)).getFunction();
        handlePriceCalculatedFunction.addEventSource(new SnsEventSource(props.getpriceCalculatedTopic()));
        props.getProductApiTable().grantReadWriteData(handlePriceCalculatedFunction);
