...
module "product_api_stock_updated_worker" {
  ...
  lambda_handler    = "ProductApi.Adapters::ProductApi.Adapters.HandlerFunctions_HandleStockUpdated_Generated::HandleStockUpdated"
  environment_variables = {
    TABLE_NAME : data.aws_ssm_parameter.product_api_table_name.value
    DD_SERVICE_MAPPING : "lambda_sns:${data.aws_ssm_parameter.stock_updated_topic_name.value}"
  }
  dd_api_key_secret_arn = var.dd_api_key_secret_arn
  dd_site               = var.dd_site
  app_version           = var.app_version
...
resource "aws_lambda_permission" "stock_updated_product_pricing_changed_sns" {
  statement_id  = "AllowExecutionFromSNS"
  action        = "lambda:InvokeFunction"
  function_name = module.product_api_stock_updated_worker.function_name
  principal     = "sns.amazonaws.com"
  source_arn    = data.aws_ssm_parameter.stock_updated_topic.value
}


resource "aws_sns_topic_subscription" "stock_updated_sns_topic" {
  topic_arn = data.aws_ssm_parameter.stock_updated_topic.value
  protocol  = "lambda"
