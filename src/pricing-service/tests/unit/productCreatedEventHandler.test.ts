...
describe("ProductCreatedEventHandler", () => {
  ...
  describe("handle", () => {
    it("should fetch product price and calculate pricing and publish an event", async () => {
      // Arrange
      const event: ProductCreatedEvent = { productId: "PROD123" };
      ...
      mockProductApiClient.getProductPrice.mockResolvedValue(29.99);
      ...
      expect(mockActiveSpan.addTags).toHaveBeenCalledWith({ "pricing.price": 29.99 });
      expect(mockPricingService.calculate).toHaveBeenCalledWith({ productId: "PROD123", price: 29.99 });
    ...
    it("should handle the case when no active span is available", async () => {
      ...
      mockProductApiClient.getProductPrice.mockResolvedValue(29.99);
      mockPricingService.calculate.mockResolvedValue([{ quantityToOrder: 5, price: 28.49 }]);
    ...
    it("should propagate errors from the product API client", async () => {
      ...
      // Act & Assert
      await expect(productCreatedEventHandler.handle(event)).rejects.toThrow("Product API unavailable");
    ...
    it("should propagate errors from the pricing service", async () => {
      ...
      // Act & Assert
      await expect(productCreatedEventHandler.handle(event)).rejects.toThrow("Failure generating prices");
