...
data "aws_iam_policy_document" "dynamo_db_read" {
  statement {
    ...
    resources = ["arn:aws:dynamodb:*:${data.aws_caller_identity.current.account_id}:table/${aws_dynamodb_table.go_inventory_api.name}", "arn:aws:dynamodb:*:*:table/${aws_dynamodb_table.go_inventory_api.name}/*"]
  }
}
...
data "aws_iam_policy_document" "dynamo_db_write" {
  statement {
    ...
    "dynamodb:DeleteItem"]
    resources = ["arn:aws:dynamodb:*:${data.aws_caller_identity.current.account_id}:table/${aws_dynamodb_table.go_inventory_api.name}", "arn:aws:dynamodb:*:${data.aws_caller_identity.current.account_id}:table/${aws_dynamodb_table.go_inventory_api.name}/*"]
  }
}
