...
resource "aws_sns_topic" "product_price_calculated" {
  name = "tfgo-price-calculated-topic-${var.env}"
}

module "product_pricing_created_handler" {
  service_name   = "GoProductPricingService"
  source         = "../../modules/lambda-function"
  entry_point    = "../src/product-pricing-service/handle-product-created"
  function_name  = "GoProductCreatedPricingHandler"
...
module "product_pricing_updated_handler" {
  service_name   = "GoProductPricingService"
  source         = "../../modules/lambda-function"
  entry_point    = "../src/product-pricing-service/handle-product-updated"
  function_name  = "GoProductUpdatedPricingHandler"
  lambda_handler = "index.handler"
  environment_variables = {
    PRICE_CALCULATED_TOPIC_ARN : aws_sns_topic.product_price_calculated.arn
    DD_SERVICE_MAPPING : "lambda_sns:${aws_sns_topic.product_price_calculated.name}"
