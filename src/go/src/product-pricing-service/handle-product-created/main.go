...
type LambdaHandler struct {
	commandHandler core.ProductCreatedEventHandler
}

func NewLambdaHandler(commandHandler core.ProductCreatedEventHandler) *LambdaHandler {
...
func (lh *LambdaHandler) Handle(ctx context.Context, request events.SNSEvent) error {
	for index := range request.Records {
		...
		var evt utils.TracedMessage[core.ProductCreatedEvent]
...
func main() {
	...
	handler := NewLambdaHandler(*core.NewProductCreatedEventHandler(adapters.NewSnsEventPublisher(*snsClient), core.PricingService{}))
