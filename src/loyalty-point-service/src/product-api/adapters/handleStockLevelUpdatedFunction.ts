...
import { StockLevelUpdatedHandler } from "../core/stock-level-updated/stockLevelUpdatedHandler";
import { StockLevelUpdatedEvent } from "../private-events/stockLevelUpdatedEvent";

const dynamoDbClient = new DynamoDBClient();
const stockLevelUpdatedHandler = new StockLevelUpdatedHandler(
...
export const handler = async (event: SNSEvent): Promise<string> => {
  ...
  for (const message of event.Records) {
    ...
    const evtWrapper: CloudEvent<StockLevelUpdatedEvent> = JSON.parse(
      message.Sns.Message
    );
