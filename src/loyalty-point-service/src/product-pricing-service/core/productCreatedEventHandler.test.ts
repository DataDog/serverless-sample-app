...
describe("pricing-product-created-handler", () => {
  beforeAll(() => {
    ...
    const mockEventPublisher = new Mock<EventPublisher>()
      .setup((instance) =>
        instance.publishPriceCalculatedEvent(
          It.Is((value: PriceCalculatedEvent) => value.productId === testId && value.priceBrackers.length === 5)
        )
      )
      .returnsAsync(true);

    const pricingService = new PricingService();
    const handler = new ProductCreatedEventHandler(
      pricingService,
