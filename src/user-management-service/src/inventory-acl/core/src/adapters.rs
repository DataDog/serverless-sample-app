...
impl EventPublisher for SnsEventPublisher {
    #[instrument(name = "publish-product-added-event", skip(self, product_added_evt))]
    async fn publish_product_added_event(
        ...
        tracing::Span::current().set_attribute(
            "peer.service",
            parse_name_from_arn(&std::env::var("PRODUCT_ADDED_TOPIC_ARN").unwrap()),
        );
        tracing::Span::current().set_attribute(
            "peer.messaging.destination",
            std::env::var("PRODUCT_ADDED_TOPIC_ARN").unwrap(),
        );
