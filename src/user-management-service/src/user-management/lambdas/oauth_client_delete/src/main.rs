...
use shared::ports::{ApplicationError, DeleteOAuthClientCommand};
use shared::response::empty_response;
use shared::tokens::TokenGenerator;
use std::env;
use std::sync::OnceLock;

#[instrument(name = "DELETE /oauth/clients/{client_id}", skip(repository, token_generator, event), fields(http.method = event.method().as_str(), http.path_group = event.raw_http_path()))]
async fn function_handler<TRepository: Repository>(
    ...
    let command = DeleteOAuthClientCommand { client_id };
