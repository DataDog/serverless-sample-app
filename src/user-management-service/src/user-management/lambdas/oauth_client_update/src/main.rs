...
#[instrument(name = "PUT /oauth/clients/{client_id}", skip(repository, token_generator, event), fields(http.method = event.method().as_str(), http.path_group = event.raw_http_path()))]
