...
module "inventory_ordering_service" {
  service_name   = "RustInventoryOrderingService"
...
resource "aws_cloudwatch_log_group" "sfn_log_group" {
  name              = "/aws/vendedlogs/states/RustInventoryOrderingServiceLogGroup-${var.env}"
  retention_in_days = 7
  lifecycle {
...
resource "aws_sfn_state_machine" "inventory_ordering_state_machine" {
  name     = "tf-rust-inventory-ordering-service-${var.env}"
...
resource "aws_ssm_parameter" "api_endpoint" {
  name  = "/rust/${var.env}/inventory-ordering/state-machine-arn"
  type  = "String"
