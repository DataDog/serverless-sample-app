...
resource "aws_sqs_queue" "product_event_acl_dlq" {
  name = "tf-rust-product-acl-dlq-${var.env}"
}

resource "aws_sqs_queue" "product_event_acl_queue" {
  name                      = "tf-rust-product-acl-${var.env}"
...
resource "aws_sns_topic" "rust_product_stock_updated" {
  name = "tf-rust-product-stock-updated-${var.env}"
}
...
module "product_acl_function" {
  ...
  environment_variables = {
    STOCK_LEVEL_UPDATED_TOPIC_ARN : aws_sns_topic.rust_product_stock_updated.arn
